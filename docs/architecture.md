...
- **Supabase Postgres** — per-user rows for skills, sessions, overrides, events, people, job applications, career targets, workout plans, workout sessions, supplement protocols, supplement intake logs, and focus feedback (RLS-scoped)
...
- [`CalendarPage`](../src/pages/CalendarPage.tsx) is an Outlook-style calendar. Navigation state (`viewMode`, current anchor date) and the selected item for the detail modal stay in the shared controller. Category / event-type / fitness-type / cooking-type sidebar filters persist client-locally (see below). It builds items via `buildCalendarItemsForRange` (with `includeFitnessHistory: true`, `includeWorkoutSchedules: true`, and `includeSupplementSchedule: true`) for the active range, filters by hidden categories / event types / fitness types / cooking types, then groups with `groupCalendarItemsByDate`. Recurring-event occurrence actions and week-view drag reschedule are wired on `CalendarPage` only; the dashboard calendar widget stays read-only.
- **Shared controller** (Phase 32): the calendar state + item derivation is extracted into the headless hook [`useCalendarController`](../src/components/calendar/useCalendarController.ts), consumed by both `CalendarPage` and the dashboard calendar widget so the calendar wiring is not duplicated. All date math stays in the pure `calendarView.ts` / `calendar.ts` modules; the hook only binds React state to them. Month/week/3-day **view mode** is persisted per surface (`dashboard` \| `calendarPage`) and viewport (`mobile` \| `desktop`) via [`calendarViewPreferences.ts`](../src/core/calendarViewPreferences.ts). **Sidebar filters** (hidden categories, event types, fitness types, cooking types) are also client-local `localStorage` — one shared key for both surfaces — so toggling a filter survives tab changes and reloads. Neither is part of the synced `AppPayload`.
