...
- Pure helpers: schedule math ([`schedule.ts`](../src/core/schedule.ts)), events ([`events.ts`](../src/core/events.ts)), people ([`people.ts`](../src/core/people.ts)), career ([`career.ts`](../src/core/career.ts)), school ([`school.ts`](../src/core/school.ts), [`schoolParse.ts`](../src/core/schoolParse.ts)), fitness ([`fitness.ts`](../src/core/fitness.ts)), supplements ([`supplements.ts`](../src/core/supplements.ts)), exercise progression ([`exerciseProgression.ts`](../src/core/exerciseProgression.ts)), muscle coverage ([`muscleMap.ts`](../src/core/muscleMap.ts), [`muscleCoverage.ts`](../src/core/muscleCoverage.ts)), unified timeline ([`timeline.ts`](../src/core/timeline.ts)), unified calendar ([`calendar.ts`](../src/core/calendar.ts)), daily focus ([`focus.ts`](../src/core/focus.ts)), daily briefing ([`briefing.ts`](../src/core/briefing.ts)), weekly review ([`review.ts`](../src/core/review.ts))
...
- Daily focus ([`focus.ts`](../src/core/focus.ts)): `buildDailyFocusSummary` aggregates skills, events, people, career, school dues, fitness, cooking, and timeline signals into ranked read-only `FocusItem` recommendations — tested in [`focus.test.ts`](../src/core/focus.test.ts). **Not persisted**; recomputed on each dashboard render. Recommendations only (no mutations, notifications, or auto-rescheduling).
...
- Weekly review ([`review.ts`](../src/core/review.ts)): `buildWeeklyReview` aggregates skills, fitness, career, school due counts, people, events, and focus feedback for the **local calendar week containing today** (Monday–Sunday) — tested in [`review.test.ts`](../src/core/review.test.ts). **Not persisted**; recomputed on dashboard and review page renders. No AI APIs, no mutations, no notifications.
...
- Calendar foundation ([`calendar.ts`](../src/core/calendar.ts)): `buildCalendarItemsForRange` converts skills, life events, people birthdays, and optional fitness history into common `CalendarItem` rows for an inclusive `YYYY-MM-DD` date range — tested in [`calendar.test.ts`](../src/core/calendar.test.ts). **Not persisted**; pure derivation with no side effects, recomputed on demand. Built for future Outlook-like week/month dashboard views; no UI in this layer.
  - **`CalendarItem`**: stable `id`, `sourceType` (`skill` \| `event` \| `people` \| `fitness` \| `career` \| `cooking` \| `school`), `sourceId`, `title`, `date`, optional `startTime`/`endTime`, derived `isTimed`/`allDay`/`isMultiDay`, theming hooks (`categoryKey`, optional `subcategoryKey`/`colorKey`/`accentColorKey`/`iconKey`), optional `description`, and a discriminated `sourceMeta` carrying original domain fields.
  - **Source mapping**: skill weekly blocks expand per date in range; life events convert directly (timed range, start-only marker, or all-day); people birthdays expand `birthdayMonthDay` per intersecting year (Feb 29 → Feb 28 in non-leap years, matching [`people.ts`](../src/core/people.ts)); workout sessions with `completedAtIso` become opt-in historical timed items (`includeFitnessHistory`); schedulable workout plans expand into `workoutScheduleBlock` items when `includeWorkoutSchedules` is true (Phase 27–30); due supplement protocols emit one all-day `supplementIntake` item per protocol per day when `includeSupplementSchedule` is true (default; Phase 49). Workout schedule and session items use `subcategoryKey: "workout"` (plan `focus` stays on `sourceMeta`); supplement items use `subcategoryKey: "supplement"`.
  ...
  - **Scheduled workouts**: [`workoutSeries.ts`](../src/core/workoutSeries.ts) + [`fitness.ts`](../src/core/fitness.ts) expand plan weekday blocks into virtual `workoutScheduleBlock` calendar rows (`subcategoryKey: workout`); completed sessions remain separate history items. Workout/skill drag and in-calendar edit remain future work (Phase 36.2+); life-event week-view drag shipped in Phase 34B and month drag/week resize in Phase 36.
  - **Supplements on calendar** (Phase 49): one fitness item per due protocol per local date (`sourceMeta.kind: supplementIntake`, all-day, `completionVisual` + `progressLabel` like `"2/4"`). Paused protocols and non-due weekdays are omitted. Sidebar **Fitness types** filter independently hides `workout` vs `supplement` (`hiddenFitnessTypes`); `nutrition` is reserved and not shown until it emits items.
  - **School on calendar** (Phase 56): `collectSchoolReminderItems` emits one `school` item per due reminder and an extra **Opens** item when `openDate` is set (`sourceMeta.kind: schoolReminder`). Timed reminders convert course IANA wall time to the browser zone (`localTimeZone`); all-day dates are not shifted. Titles stay the reminder name (never a URL). `subcategoryKey` is the reminder kind. `includeSchoolReminders` defaults true. Sidebar **School types** filter independently hides assignment / quiz / exam / project / study / reading / other. Default subcategory colors: assignment sky, quiz yellow, exam rose, project fuchsia, study emerald, reading lime (`other` inherits teal). Optional per-course `colorToken` is copied to `CalendarItem.accentColorKey` and rendered as a small right-to-left gradient overlay so the kind fill stays visible.

### Calendar UI (`CalendarPage` + `components/calendar`)

- [`CalendarPage`](../src/pages/CalendarPage.tsx) is an Outlook-style calendar. Navigation state (`viewMode`, current anchor date) and the selected item for the detail modal stay in the shared controller. Category / event-type / fitness-type / cooking-type / school-type sidebar filters persist client-locally (see below). It builds items via `buildCalendarItemsForRange` (with `includeFitnessHistory: true`, `includeWorkoutSchedules: true`, and `includeSupplementSchedule: true`) for the active range, filters by hidden categories / event types / fitness types / cooking types / school types, then groups with `groupCalendarItemsByDate`. Recurring-event occurrence actions and week-view drag reschedule are wired on `CalendarPage` only; the dashboard calendar widget stays read-only.
- **Shared controller** (Phase 32): the calendar state + item derivation is extracted into the headless hook [`useCalendarController`](../src/components/calendar/useCalendarController.ts), consumed by both `CalendarPage` and the dashboard calendar widget so the calendar wiring is not duplicated. All date math stays in the pure `calendarView.ts` / `calendar.ts` modules; the hook only binds React state to them. Month/week/3-day **view mode** is persisted per surface (`dashboard` \| `calendarPage`) and viewport (`mobile` \| `desktop`) via [`calendarViewPreferences.ts`](../src/core/calendarViewPreferences.ts). **Sidebar filters** (hidden categories, event types, fitness types, cooking types, school types) are also client-local `localStorage` — one shared key for both surfaces — so toggling a filter survives tab changes and reloads. Neither is part of the synced `AppPayload`.
- View math is isolated in the pure, tested [`calendarView.ts`](../src/core/calendarView.ts): `computeMonthVisibleRange` / `computeWeekRange`, `buildMonthGrid` (6×7, Sunday→Saturday, `inCurrentMonth`/`isToday` flags), `buildWeekGrid` (7 columns), `shiftMonth` / `shiftWeek` / `monthAnchorFromKey`, `filterItemsByHiddenCategories` / `filterItemsByHiddenEventSubcategories` / `filterItemsByHiddenFitnessTypes`, `splitDayItems`, `limitDayItems` (month "+N more"), `computeTimedItemLayout` (week block vertical placement, minimum height for start-only items), `computeTimedOverlapLayouts` (week-view timed overlap lanes — overlapping items in a day column are grouped, assigned to the first free lane, and laid out side-by-side with `leftPercent`/`widthPercent`/`zIndex`; back-to-back items do not overlap), and label helpers (`formatMonthTitle`, `formatWeekRangeTitle`, `formatHourLabel`, `formatItemTimeLabel`, `formatSourceTypeLabel`). Tested in [`calendarView.test.ts`](../src/core/calendarView.test.ts).
- Components ([`components/calendar/`](../src/components/calendar/)): `CalendarToolbar` (prev/next/today + month↔week toggle), `CalendarCategorySidebar` (per-`CalendarCategoryKey` show/hide toggles plus Event types, Fitness types, Cooking types, and School types; hidden categories dimmed; render-only), `MonthView` (compact `CalendarItemPill`s + "+N more" → jump to week), `WeekView` (all-day row + 24h timeline with absolutely-positioned `CalendarEventBlock`s, optional pointer drag for one-time timed events, and a current-time line), `CalendarItemDetailModal` (detail + recurring occurrence actions on `CalendarPage`; supplement progress + **Open in Fitness**; school timezone caption, tappable link labels, and **Open in Career** → School course; Escape + overlay-close, `role="dialog"`), and `CalendarSettingsSection` (collapsible color/alias editor). All colors come from `resolveCalendarItemColor` using the optional persisted `calendarPreferences`.
...
- Calendar colors ([`calendarColors.ts`](../src/core/calendarColors.ts)): pure, dependency-free resolution of a display color (and category display label) for a calendar item — tested in [`calendarColors.test.ts`](../src/core/calendarColors.test.ts). No React, storage, or Supabase in this module; total functions that never throw and never mutate inputs. Consumed by calendar views and the settings UI on demand.
  ...
  - **Resolution precedence** (`resolveCalendarItemColorToken` / `resolveCalendarItemColor`): **item override** (`CalendarItem.colorKey` when a valid token) → **subcategory** (pref or default for `"category:subcategory"`, e.g. `event:birthday → amber.base`) → **category** (pref or built-in default: `skill` indigo, `event` red, `people` pink, `fitness` green, `career` violet) → **fallback**. Invalid/unknown tokens at any step are ignored and fall through. Input is a structural subset (`categoryKey` / `subcategoryKey?` / `colorKey?` / `accentColorKey?`), so the module stays decoupled from `calendar.ts`. `colorKey` replaces the fill; `accentColorKey` does not — `resolveCalendarItemColorStyle` keeps the resolved fill and layers a right-to-left gradient when the accent token is valid (school course color).
...
[`DashboardPage`](../src/pages/DashboardPage.tsx) receives `skills`, `sessions`, `events`, `people`, `jobApplications`, `careerTarget`, `workoutPlans`, `workoutSessions`, `supplementProtocols`, `supplementIntakeLogs`, `focusFeedback`, `calendarPreferences`, `gamificationState`, focus feedback callbacks, page-navigation callbacks, the `onAcknowledgeGlobalLevel` / `onDismissAchievement` gamification callbacks, `onAddSession`, Phase 45 `onToggleTodayExercise` / `onSetTodayExerciseWeight`, and Phase 48 `onUpsertSupplementIntake` from `App`, runs pure calculations in [`dashboardStats.ts`](../src/core/dashboardStats.ts), [`progression.ts`](../src/core/progression.ts), [`progressionSnapshot.ts`](../src/core/progressionSnapshot.ts), [`focus.ts`](../src/core/focus.ts), [`focusFeedback.ts`](../src/core/focusFeedback.ts), [`briefing.ts`](../src/core/briefing.ts), [`review.ts`](../src/core/review.ts), [`events.ts`](../src/core/events.ts), [`people.ts`](../src/core/people.ts), [`career.ts`](../src/core/career.ts), [`fitness.ts`](../src/core/fitness.ts), and [`supplements.ts`](../src/core/supplements.ts). The Phase 35 gamification panel reuses the calendar/domain inputs already passed; only the `gamificationState` prop and the two ack callbacks were added. `onOpenFitness` may pass an optional `FitnessFocus` (`workout` `{ date, planId }` or `supplement` `{ date, protocolId }`) so the Fitness today rail is ready.
...
- **Left rail (do-now):** `DailyFocusSection` → calendar **category filters** (`CalendarCategorySidebar`, session-only show/hide for categories, Event types, and Fitness types) → `DashboardQuickActions` (deep-link nav buttons).
...
- **Right rail (briefing/context):** `DailyBriefingSection` → `UpcomingEventsSection` → `WeeklyReviewSection` → `CareerActionsSection` → `FitnessSummarySection` (week summary, last workout, today’s scheduled plan exercises with Complete + weight, and today’s due supplement dose taps; **Open in Fitness** deep-links via `fitnessFocus`) → `PeopleRemindersSection` (the domain alerts auto-hide when empty).
...
- **Manual CRUD:** [`SchoolSection`](../src/components/school/SchoolSection.tsx) creates/edits/deletes courses, reminders, grade categories, and graded items. Each course block shows a policy glance (staff, emails, late days, office hours, extra credit) even when empty. Reminder forms show a source-zone + local due caption.
...
- **`EventType`** does not include `"school"`. Existing `events.type = 'school'` (and legacy `"deadline"`) migrate to `"other"` on load (`migrateLegacyEventTypes`) and in SQL (`20260824100000_school_domain.sql`). School due dates are their own calendar category (`school`) with kind colors and filters.
- **`SchoolCourse` / `SchoolReminder` / `SchoolGradedItem`** live on `AppPayload` as sibling arrays. Nested staff, office hours, late policy, grade categories, and reminder links are jsonb arrays validated in [`dbMappers.ts`](../src/core/dbMappers.ts). Optional `SchoolCourse.colorToken` is a calendar palette token (`blue.base`, etc.) stored as `school_courses.color_token`; it overlays the reminder-kind color on the calendar and does not change Calendar settings.
- Pure helpers in [`school.ts`](../src/core/school.ts): CRUD-shaped create/upsert/remove, reminder fingerprints (`kind|normalized title|due date`), URL auto-labels (hostname + last path segment), IANA timezone conversion (source course zone → local wall time; all-day dates are not shifted), GT letter scale. Tested in [`school.test.ts`](../src/core/school.test.ts).
- **What-if grades (Phase 58):** pure [`schoolGrades.ts`](../src/core/schoolGrades.ts) computes current weighted % (ungraded / 0% weight / extra-credit-0% do not consume the required pie), projected min (0% remaining) and max (100% remaining) with GT letters, need-for-A average on remaining work, and per-remaining-item floors (equal-remaining average plus the score that still yields an A if every other remaining item is 100%). Points-weighted inside a category when every required item has `maxScore`; otherwise equal split of category weight. Extra credit can raise the total above 100%. The course grades panel ([`SchoolGradesEditor`](../src/components/school/SchoolGradesEditor.tsx) + [`SchoolGradeWhatIfPanel`](../src/components/school/SchoolGradeWhatIfPanel.tsx)) groups scores by category so remaining work is visible at a glance.
- **Persistence:** `school_courses`, `school_reminders`, `school_graded_items` (RLS, `updated_at` triggers). Optional `school_courses.color_token` is a palette token allowlisted in SQL and mappers. Reminders and graded items FK to courses (`ON DELETE CASCADE`); graded `reminder_id` is `ON DELETE SET NULL`.
- **Daily Focus (Phase 59):** `collectSchoolFocusItems` rehomes the old “school due within 3 days” signal onto `SchoolReminder` dues (local date after course-zone conversion; all-day dates are not shifted). Items live in the career focus category with `school_due_today` / `school_due_soon`, CTA `open_career`, and `CareerFocus` `{ kind: "school", courseId }` (**Open in School**). Briefing/review add one-liners only (upcoming dues in the next 3 days; this-week / next-week due counts). No school XP.
...
- Pure helpers in [`fitness.ts`](../src/core/fitness.ts): focus labels, search/sort, week summary (including optional duration totals), plan→session copy (`createSessionDraftFromPlan` / `createLiveSessionFromPlan`), live-session toggles, finish defaults (`applyWorkoutFinishDefaults`, 60-minute fallback), dashboard loggers (`buildDashboardWorkoutLoggers`, `dashboardToggleExercise`, `dashboardSetExerciseWeight`), occurrence expansion/completion matchers, recent exercise name collection (`collectRecentExerciseNames`) for the progression catalog (completed sessions first, plans as known-name fallback). Session history grouping lives in [`workoutHistory.ts`](../src/core/workoutHistory.ts).
...
- Muscle anatomy ([`muscleMap.ts`](../src/core/muscleMap.ts) + [`muscleCoverage.ts`](../src/core/muscleCoverage.ts)): exercise names map to **primary** muscles only (bicep curl → biceps, not the whole arm). Weekly coverage is derived from scheduled plan occurrences vs completed session/exercise taps; the live chart resets on the local Monday and past weeks stay available as snapshots because session dates are the source of truth (no denormalized snapshot table). Monthly heatmap percent is `completed / scheduled` per muscle when the muscle was planned, otherwise relative frequency. UI: [`MuscleAnatomyChart`](../src/components/fitness/MuscleAnatomyChart.tsx) on Fitness **Progress** (front/back SVG, weekly dual colors, weekly archive, monthly heatmap).
...
- **UI:** [`WorkoutPlanForm`](../src/components/fitness/WorkoutPlanForm.tsx) + [`WorkoutPlanScheduleSection`](../src/components/fitness/WorkoutPlanScheduleSection.tsx) edit weekly blocks and schedule bounds (indefinite persists as omitted `scheduleSeries`; Date Range with a start and no end persists as `{ mode: "indefinite", startDate }` and stays active from that date onward). [`WorkoutSessionForm`](../src/components/fitness/WorkoutSessionForm.tsx) packs date/start/focus/plan/duration into a wrapping row; notes stay full-width. [`LiveWorkoutLogger`](../src/components/fitness/LiveWorkoutLogger.tsx) / [`PlanLiveLogger`](../src/components/fitness/PlanLiveLogger.tsx) power the today rail and plan **Open** / **Log session** views (per-exercise Complete taps; first tap stamps `startedAtIso` if unset). Plan and session lists use a responsive card grid ([`workoutGalleryGrid`](../src/ui/appStyles.ts)); session history is grouped by week (<28 days), month (<365 days), then year ([`workoutHistory.ts`](../src/core/workoutHistory.ts)). [`ExerciseProgressionChart`](../src/components/fitness/ExerciseProgressionChart.tsx) and [`MuscleAnatomyChart`](../src/components/fitness/MuscleAnatomyChart.tsx) live on the Fitness **Progress** section. [`FitnessSummarySection`](../src/components/dashboard/FitnessSummarySection.tsx) offers the same Complete + weight path without opening Fitness. First mutation upserts an in-progress session via `upsertWorkoutSession`; **Finish** prompts for an optional completion time, then [`applyWorkoutFinishDefaults`](../src/core/fitness.ts) fills start (first tap) and duration (elapsed or 60 minutes). In-progress sessions do not emit a new calendar block until `completedAtIso` is set. [`FitnessPage`](../src/pages/FitnessPage.tsx) switches **Workouts | Progress | Supplements**; supplement protocols use [`SupplementProtocolForm`](../src/components/fitness/SupplementProtocolForm.tsx) (optional loading days resolve to `endDate` + maintenance `startDate`) and [`SupplementDoseRow`](../src/components/fitness/SupplementDoseRow.tsx) for per-dose taps on Fitness and the dashboard. First dose tap upserts today’s `SupplementIntakeLog` via `upsertSupplementIntake` (unique per protocol+date).
- **Focus phase (workout + cooking):** localStorage `pa.focusPhase.v1` ([`focusPhase.ts`](../src/core/focusPhase.ts) / [`focusPhaseStorage.ts`](../src/core/focusPhaseStorage.ts)) remembers which in-progress workout or cook should resume after a tab/window close. **Log session** on a plan (or **Focus mode** on sessions / **Start Cooking** on a recipe) turns it on; an **Exit focus** control turns it off without abandoning the session. While workout focus is on, live-logger typing is snapshotted in the phase draft and restored on return; exiting discards that draft. A [`FocusPhaseBanner`](../src/components/layout/FocusPhaseBanner.tsx) appears in `AppShell` when navigating to another page.
- **Consumers:** calendar (one merged fitness block per plan occurrence: planned / in progress / completed, plus one all-day supplement item per due protocol per day), daily focus (`fitness_workout_scheduled_today` / remaining supplement doses / missed yesterday), briefing focus paragraph, weekly review adherence metrics, dashboard fitness summary (scheduled today + quick-complete + supplement dose taps), body XP for complete supplement days.
- Deleting a plan clears `planId` on linked sessions in the same `commit` (mirrors person unlink on events). Deleting a supplement protocol drops its intake logs.
- **Supplements (Phase 47–51):** [`SupplementProtocol`](../src/core/model.ts) templates (`phases[]` with derived loading/maintenance windows) and [`SupplementIntakeLog`](../src/core/model.ts) (one row per protocol per local date, `doses[]` with optional `takenAtIso`). Pure helpers in [`supplements.ts`](../src/core/supplements.ts). Tables `supplement_protocols` / `supplement_intake_logs` (migration `20260818000000_supplement_tracker.sql`). `FitnessFocus` is a discriminated union (`workout` | `supplement`) so dashboard **Open in Fitness**, focus CTAs, and calendar modal deep-link to a protocol row. Calendar emits `supplementIntake` items under Fitness with `workout` / `supplement` type filters (stale `fitness:push` color prefs stay in stored JSON and are ignored). Daily Focus nags remaining doses (not when there are zero protocols); briefing/review mention incomplete days and weekly adherence under Fitness. Full adherence days grant small `body` XP (`supplement_adherence_day:{protocolId}:{date}`, `BONUS_XP.supplementAdherenceDay`, under `MAX_BONUS_XP_PER_DAY`); creating/editing protocols and in-progress days grant 0. Protocol cards show a consecutive complete-due-day streak. Achievement `supplement_days_7` (`supplement_adherence_days_gte`) unlocks after 7 complete days.
- Future phases (not implemented): calorie tracker (`FitnessType: "nutrition"` reserved), biweekly/monthly `RecurrenceRule` on plans, series exception UI.
...
- Unit tests live next to core modules (e.g. `dbMappers.test.ts`, `career.test.ts`, `fitness.test.ts`, `exerciseProgression.test.ts`, `muscleMap.test.ts`, `muscleCoverage.test.ts`, `focus.test.ts`, `briefing.test.ts`, `review.test.ts`, `dashboardStats.test.ts`, `progression.test.ts`, `recurrence.test.ts`, `skillSeries.test.ts`, `theme.test.ts`)
