...
- **`SchoolCourse` / `SchoolReminder` / `SchoolGradedItem`** live on `AppPayload` as sibling arrays. Nested staff, office hours, late policy, grade categories, and reminder links are jsonb arrays validated in [`dbMappers.ts`](../src/core/dbMappers.ts).
...
- **Persistence:** `school_courses`, `school_reminders`, `school_graded_items` (RLS, `updated_at` triggers). Reminders and graded items FK to courses (`ON DELETE CASCADE`); graded `reminder_id` is `ON DELETE SET NULL`.
