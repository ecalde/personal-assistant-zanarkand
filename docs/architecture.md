...
- [`CalendarPage`](../src/pages/CalendarPage.tsx) is an Outlook-style calendar with local-only UI state (no persistence): `viewMode` (`month` default \| `week`), the current anchor date, a `hiddenCategories` set, `hiddenEventSubcategories`, `hiddenFitnessTypes`, and the selected item for the detail modal. It builds items via `buildCalendarItemsForRange` (with `includeFitnessHistory: true`, `includeWorkoutSchedules: true`, and `includeSupplementSchedule: true`) for the active range, filters by hidden categories / event types / fitness types, then groups with `groupCalendarItemsByDate`. Recurring-event occurrence actions and week-view drag reschedule are wired on `CalendarPage` only; the dashboard calendar widget stays read-only.
