...
| **Fitness** | Workout plans (templates), live/in-progress and completed sessions, dashboard quick-complete, weight-progression chart |
...
| 37C.1 | **Settings Theme Mode Participation** | Settings page now follows Light / Dark / System like the rest of the app. `panelBackground` in [`theme.ts`](../../src/core/theme.ts) is mode-aware (light translucent glass / dark navy glass). [`settingsStyles.ts`](../../src/components/settings/settingsStyles.ts) migrated from fixed dark literals to mode-aware `--aether-*` tokens (bg, surface*, border, panel-bg, text, accent). Glassmorphism + accent glow preserved; profile badge/preview CTA use fixed on-accent text for contrast. [`ThemePreviewCard`](../../src/components/settings/ThemePreviewCard.tsx) preview inset uses resolved tokens. `theme.test.ts` Phase 37C.1 block. No layout/schema/dependency changes. |
| 37C.2 | **Text Token Adoption (Dark Mode Readability)** | Extended `theme.ts` with a semantic text hierarchy (`--aether-text-primary/secondary/muted/disabled/on-accent`; `--aether-text` aliases primary) and mode-aware semantic chip tokens (success/danger/warning/info/marker/neutral). Centralized migration in [`appStyles.ts`](../../src/ui/appStyles.ts): replaced opacity-based text dimming and hardcoded hex text with semantic tokens; status/event/marker chips flip tints in dark mode; shared `textMuted`/`ghostBtn`/`actionBtn`/`metaText` helpers; global `button { color: inherit }` in `GlobalEffectStyles`. ~50 component/page files migrated. `theme.test.ts` Phase 37C.2 block. No layout/spacing/typography changes. |
| 37D | **Global Visual Effects** | Promoted the four interface effects (Ambient Particles, Animated Borders, Magical Energy Trails, Floating Runes) from Settings-local code into one centralized engine. Pure [`themeEffects.ts`](../../src/core/themeEffects.ts) `resolveEffectSettings(prefs, env)` is the single source of truth for which effects render and at what density (accent-intensity-driven particle density, low/medium/high performance tiers, mobile/touch degradation, reduced-motion off-switch); optional backward-compatible `effectPerformance` (default `medium`) + `reducedMotion` (default `system`) fields on `AppearancePreferences`. New [`src/components/effects/`](../../src/components/effects/) — `GlobalEffectStyles` (keyframes + reusable `.aether-animated-border` class injected once), `ThemeEffectsLayer` (single `aria-hidden`, `pointer-events:none` overlay mounted once in `App.tsx`) wrapping `AmbientParticlesLayer` / `FloatingRunesLayer` / `EnergyTrailLayer` (new cursor trail, desktop/precise-pointer only) / `AnimatedBorderSystem` (root `data-aether-borders` flag). Effects read `--aether-*` tokens so they follow the active mode + profile + intensity. Settings gains **Effect Performance** + **Reduced Motion** controls and no longer paints its own particles/runes; `SettingsPage` keyframes/arrays removed (no duplication). `themeEffects.test.ts` + `theme.test.ts` Phase 37D normalization block. No new dependencies, no schema changes, no layout redesign. |
| 42 | **Live session model** | `startedAtIso` on sessions, per-exercise `completedAtIso` / `sourceExerciseId`, `started_at` migration, mappers, CRUD honors caller stamps, complete-only occurrence/quest/XP helpers, start time on the session form. |
| 43 | **Today rail + live logger** | Compact exercise grid, Fitness today rail with auto-save taps, Finish vs mark-all, `upsertWorkoutSession`. |
...
| **37C.1** | ✅ Shipped | **Settings Theme Mode Participation** — Settings page follows Light / Dark / System via the same `--aether-*` tokens as the rest of the app; mode-aware `--aether-panel-bg`; `settingsStyles.ts` migrated off fixed dark literals. **Deliverable met:** Theme Mode control and Settings chrome stay visually consistent. |
| **37C.2** | ✅ Shipped | **Text Token Adoption (Dark Mode Readability)** — semantic text hierarchy (`--aether-text-primary/secondary/muted/disabled/on-accent`) + mode-aware semantic chip tokens; opacity-based text dimming eliminated; ~50 files migrated via `appStyles.ts`. **Deliverable met:** dashboard labels, buttons, muted/helper text readable in Dark Mode. |
| **37D** | ✅ Shipped | **Global Visual Effects** — the four effects (Ambient Particles, Animated Borders, Magical Energy Trails, Floating Runes) promoted from Settings-local code into one centralized engine mounted once in `App.tsx`; pure `themeEffects.ts` resolver (`resolveEffectSettings`); accent-intensity-driven particle density; low/medium/high performance tiers; mobile/touch degradation; `prefers-reduced-motion` off-switch; new cursor energy trail (desktop only); no duplicated implementations; no new dependencies. **Deliverable met:** effects render app-wide, configurable globally via Settings. |
| **37E** | Planned (next) | **Appearance Cloud Sync** — `appearance_preferences` Supabase singleton; sync profile, intensity, **mode**, **effects**, **performance**; local fallback + backward compatibility for existing localStorage users. **Deliverable:** theme follows the account across devices. (Previously numbered 37C; moved after modes + effects so the synced preference shape is finalized first.) |
...
**Deliverable met:** `Azure + Light` / `Azure + Dark` / `Emerald + Light` / … all render correctly app-wide; mode is selectable and persisted locally (`pa.appearance.v1`). Settings page participation completed in **37C.1**.

### Phase 37C.1 — Settings Theme Mode Participation · ✅ Shipped

**Goal:** Make the Settings page follow Light / Dark / System using the same mode-aware Aether tokens as the rest of the application (fixes inconsistency where Theme Mode is controlled inside Settings but the page stayed deep navy).

**Delivered:** mode-aware `panelBackground` in [`theme.ts`](../../src/core/theme.ts) (`LIGHT_BASE`: light translucent glass; `DARK_BASE`: navy glass) · [`settingsStyles.ts`](../../src/components/settings/settingsStyles.ts) migrated to `--aether-bg`, `--aether-surface-*`, `--aether-border`, `--aether-panel-bg`, `--aether-text*`, `--aether-panel-border` (glassmorphism + accent glow preserved; fixed on-accent text for profile badge / preview CTA) · [`ThemePreviewCard`](../../src/components/settings/ThemePreviewCard.tsx) preview inset uses resolved tokens · `theme.test.ts` Phase 37C.1 block.

**Deliverable met:** Settings chrome matches the selected Theme Mode; Dark Mode preserves the fantasy-futuristic deep-navy feel; Light Mode is readable and consistent with Dashboard/domain pages.

### Phase 37C.2 — Text Token Adoption (Dark Mode Readability) · ✅ Shipped

**Goal:** Fix dark-mode text readability systematically — no one-off color patches. Replace hardcoded hex text and opacity-based dimming with a semantic Aether text token hierarchy that flips with theme mode.

**Delivered:** Extended [`theme.ts`](../../src/core/theme.ts) with `--aether-text-primary`, `--aether-text-secondary`, `--aether-text-muted`, `--aether-text-disabled`, `--aether-text-on-accent` (plus `--aether-text` alias) and mode-aware semantic chip tokens (`--aether-chip-*`). Centralized shared text/chip styles in [`appStyles.ts`](../../src/ui/appStyles.ts); migrated ~50 component/page files from `opacity: 0.X` text dimming to semantic tokens; nav/buttons/inputs explicitly set primary text; global `button { color: inherit }` in [`GlobalEffectStyles`](../../src/components/effects/GlobalEffectStyles.tsx). `theme.test.ts` Phase 37C.2 block.

**Constraints honored:** Light mode readability preserved · no layout/spacing/typography changes · semantic error surfaces + auth screen remain documented exceptions.

**Deliverable met:** Dashboard labels (categories, context, expiration, buttons, muted text) and app-wide secondary text are readable in Dark Mode.

### Phase 37D — Global Visual Effects · ✅ Shipped

**Goal:** Centralize the four interface effects into one engine mounted once, configurable globally, performance-aware, mobile-graceful, and reduced-motion ready. Full plan: [phase-37d-global-visual-effects.md](./phase-37d-global-visual-effects.md).

**Delivered:** pure [`themeEffects.ts`](../../src/core/themeEffects.ts) (`resolveEffectSettings(prefs, env)` + `resolveReducedMotion`) as the single decision layer; optional backward-compatible `effectPerformance` (`low`/`medium`/`high`, default `medium`) + `reducedMotion` (`system`/`on`/`off`, default `system`) on `AppearancePreferences` with normalization. New [`src/components/effects/`](../../src/components/effects/): `GlobalEffectStyles` (drift/float/pulse/border keyframes + reusable `.aether-animated-border` class + `prefers-reduced-motion` rule injected once), `ThemeEffectsLayer` (single `aria-hidden`, `pointer-events:none` overlay mounted once in `App.tsx`) composing `AmbientParticlesLayer` + `FloatingRunesLayer` (behind content) + `EnergyTrailLayer` (cursor-follow trail, above content) + `AnimatedBorderSystem` (root `data-aether-borders` flag). Effects read only `--aether-*` tokens. Settings adds **Effect Performance** + **Reduced Motion** segmented controls and stops painting its own particles/runes. `themeEffects.test.ts` + `theme.test.ts` Phase 37D normalization block.

**Decisions:** particle density is driven by **Accent Intensity** (then scaled by tier/mobile); **Magical Energy Trails** implemented as a desktop/precise-pointer-only cursor trail (auto-disabled on touch); animated borders use a reusable opt-in class applied to a curated set of panels (dashboard main card + Settings preview) to avoid excessive repainting; performance tiers named `low`/`medium`/`high` per the requirement.

**Constraints honored:** No duplicated implementations · centralized + globally configurable · respects performance tiers · degrades on mobile/touch · honors `prefers-reduced-motion` (particles+trails off, borders+runes static) · `App.tsx` orchestration-only · no new dependencies, schema, or layout redesign.

**Deliverable met:** Ambient particles, animated borders, energy trails, and floating runes render consistently app-wide from a single pure resolver.
