...
*Last updated: 2026-05-31 — Phase 37D (Global Visual Effects) shipped: four effects centralized into one engine (pure `themeEffects.ts` + `src/components/effects/`), mounted once in `App.tsx`; accent-intensity particle density, low/medium/high tiers, mobile/touch degradation, reduced-motion off-switch, new cursor energy trail. Next: 37E (Appearance Cloud Sync).*
