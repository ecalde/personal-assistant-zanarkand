---
name: Phase 27 Workout Scheduling
overview: Architecture plan to make WorkoutPlans schedulable using a skill-primary hybrid model (WeeklySchedule + schedule-series bounds), virtual occurrence expansion for calendar, day-level session completion linking, and a phased path through persistence, UI, and calendar integration—foundation only in Phase 27 (no schema/code).
todos:
  - id: write-doc
    content: "After approval: save plan to docs/plans/phase-27-workout-scheduling-foundation.md"
    status: completed
  - id: phase-27a-pure
    content: "Phase 27A: model.ts + workoutSeries.ts + fitness expansion helpers + tests"
    status: completed
  - id: phase-28-db
    content: "Phase 28: migration schedule/schedule_series on workout_plans + dbMappers"
    status: completed
  - id: phase-29-ui
    content: "Phase 29: WorkoutPlanForm schedule UI + App.tsx persistence semantics"
    status: completed
  - id: phase-30-calendar
    content: "Phase 30: calendar.ts workoutScheduleBlock + dashboard/focus/review consumers"
    status: completed
isProject: false
---
